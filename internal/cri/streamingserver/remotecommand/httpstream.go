...
import (
	...
	api "github.com/basuotian/containerd/v2/internal/cri/streamingserver/v1"
...
func NewOptions(req *http.Request) (*Options, error) {
	...
	if tty && stderr {
		...
		log.L.Info("Access to exec with tty and stderr is not supported, bypassing stderr")
		stderr = false
	}
...
func createHTTPStreamStreams(req *http.Request, w http.ResponseWriter, opts *Options, supportedStreamProtocols []string, idleTimeout, streamCreationTimeout time.Duration) (*connectionContext, bool) {
	...
	case "":
		log.L.WithField("protocol", remotecommandconsts.StreamProtocolV1Name).Info("Client did not request protocol negotiation. Falling back")
		fallthrough
